...
const sizes_ratio = [0.8, 1, 1.2, 1.4, 1.6, 1.8]
const diff = [
	[8, 9],
	[-17, 2],
	[6, -5],
...
function scaleBoard(size) {
	...
	if (horizontal_map) {
		...
		scene.canvas.setHeight(
			(winheight + BOARD_MARGIN / 2) * sizes_ratio[scene.cursize]
		)
	} else {
		scene.canvas.setWidth(
			(winwidth + BOARD_MARGIN / 2) * sizes_ratio[scene.cursize]
		)

		scene.canvas.setHeight(
			(winheight + BOARD_MARGIN) * sizes_ratio[scene.cursize]
		)
	}
}
/**
	...
	show() {
		...
		for (let i = 0; i < this.scope.length; ++i) {
			this.scopeTiles[i].set({
				left:
					coordinates[this.scope[i]].x + BOARD_MARGIN + PROJ_DIFF[this.owner],
				top:
					coordinates[this.scope[i]].y + BOARD_MARGIN + PROJ_DIFF[this.owner],
				visible: true,
				evented: true,
			})
		}
		this.icon.set({
			...
			top:
				coordinates[this.scope[0]].y +
				25 +
				BOARD_MARGIN +
				PROJ_DIFF[this.owner],
			visible: true,
			evented: true,
		})
		scene.canvas.renderAll()
...
class Scene {
	...
	destroyProj(UPID) {
		try {
			let toDestroy = this.activeProjectileList.find(
				(proj) => proj.UPID === UPID
			)
			toDestroy.remove()
			this.activeProjectileList = this.activeProjectileList.filter(
				(proj) => proj.UPID !== UPID
			)
		} catch (e) {
			console.log(e)
	...
	showObjects() {
		...
		for (let i = 0; i < $(".visualeffect").toArray().length; ++i) {
			let img = new fabric.Image(
				document.getElementById("effect" + String(i + 1)),
				{
					evented: false,
					opacity: 0,
					left: 0,
		...
		for (let i = 0; i < 4; ++i) {
			let tileselectimg = new fabric.Image(
				document.getElementById("tileselectimg"),
				{
					evented: false,
					opacity: 0,
					left: 0,
		...
		for (let i = 0; i < this.tiles.length; ++i) {
			...
			this.tiles[i].set({
				top: coordinates[i].y + BOARD_MARGIN,
				left: coordinates[i].x + BOARD_MARGIN,
			})
		}
		...
		for (let i = 0; i < game.S; ++i) {
			let img = document.getElementById(
				"playerimg" + (game.player_champlist[i] + 1)
			)
	...
	movePlayer(actualdice, count, pos, turn) {
		...
		if (!nameTextTeamSet) {
			for (let i = 0; i < game.S; ++i)
				if (game.teams[i] === 0) {
					this.nametexts[i].set("fill", "#ffa1a1")
				} else if (game.teams[i] === 1) {
					this.nametexts[i].set("fill", "#a6c8ff")
				}

			nameTextTeamSet = true
		}
		...
		switch (type) {
			...
			case "revive":
				...
				this.effectlist[15]
					...
					.bringToFront()
				this.effectlist[15].animate("opacity", 0, {
					onChange: this.canvas.renderAll.bind(this.canvas),
					duration: 1300,
					easing: fabric.util.ease.easeInCubic,
				})
				this.effectlist[15].animate("scaleX", scale + 1.7, {
					onChange: this.canvas.renderAll.bind(this.canvas),
					duration: 400,
					easing: fabric.util.ease.easeOutCubic,
				})
				this.effectlist[15].animate("scaleY", scale + 1.7, {
					onChange: this.canvas.renderAll.bind(this.canvas),
					duration: 400,
					easing: fabric.util.ease.easeOutCubic,
				})
	...
	changeShield(target, shield, change, indicate) {
		...
		if (shield === 0) $(shieldframe[ui]).hide()
		else $(shieldframe[ui]).css("display", "inline-block")
		...
		this.shieldindicator[target]
			.set({ top: y, left: x, opacity: 1 })
			.bringToFront()

		this.shieldindicator[target].set("text", String(change))
		...
		this.shieldindicator[target].animate("opacity", 0, {
			onChange: this.canvas.renderAll.bind(this.canvas),
			duration: time,
			easing: fabric.util.ease.easeOutCubic,
		})
		this.shieldindicator[target].animate("top", y - 50, {
			onChange: this.canvas.renderAll.bind(this.canvas),
			duration: time,
			easing: fabric.util.ease.easeOutCubic,
		})
	...
	animateHP(target, hp, maxhp, change, skillfrom, type) {
		...
		if (skillfrom > 0) {
			...
			this.line
				.set({ opacity: 1, x1: pos1.x, y1: pos1.y, x2: pos2.x, y2: pos2.y })
				.bringToFront()
		...
		if (change < 0 || type === "nodmg_hit" || type === "shield") {
			...
			this.heal
				.set({ opacity: 1, left: pos.x, top: pos.y - 40 * sc, scaleY: sc })
				.bringToFront()
	...
	indicateEffect(target, effect, num) {
		...
		this.effectindicator[num]
			.set({ top: y - 50 - num * 50, left: x - 50, opacity: 1 })
			.bringToFront()
	...
	showSelectedTileSingle(index) {
		let pos = this.getTilePos(index)
		this.tileselectimgs[0]
			.set({ left: pos.x, top: pos.y, opacity: 1 })
			.bringToFront()
		this.canvas.renderAll()
	}
	...
	hidePlayer(target, pos) {
		...
		this.playerimgs[target].set({
			visible: false,
			top: coordinates[pos].y + BOARD_MARGIN,
			left: coordinates[pos].x + BOARD_MARGIN,
		})
	...
	getPlayerPos(target) {
		return {
			x: this.playerimgs[target].get("left"),
			y: this.playerimgs[target].get("top"),
		}
	}
	getTilePos(tile) {
		return {
			x: Map.coordinates[tile].x + BOARD_MARGIN,
			y: Map.coordinates[tile].y + BOARD_MARGIN,
		}
	}
}
