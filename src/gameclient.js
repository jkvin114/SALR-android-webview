...
socket.on("changeshield", function (val) {
	if (val == null) return

	scene.changeShield(val.turn, val.shield, val.change, val.indicate)
})
...
socket.on("godhand_target", function (targets) {
	if (game.ismyturn || !targets) {
		$("#largetext").html(
			chooseLang(
				"God`s hand <br> Choose a player to move",
				"신의손<br>이동시킬 플레이어 선택"
			)
		)
...
socket.on("respawn", function (who, respawnPos, isRevived) {
	if (who == null) return
	playerRespawn(who, respawnPos, isRevived)
})
