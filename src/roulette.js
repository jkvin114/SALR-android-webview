  ...
  const CASINO=1
  const TRIAL=2
  const casinocolor = ['#f5dd42','#f5b642','#f5dd42','#f5b642','#f5dd42','#f5b642'];
  const trialcolor=['#fbc','#f88','#fbc','#f88','#fbc','#f88'];
  const casinolabel=['+100$','Money doubles','Speed for 2 turns','HP halves','Money halves','Stun, HP-50']
  const triallabel=['-100$','Summon every player','Move to nearest player','Death!','Stun, HP-50','50% chance of death \nfor every player']
  ...
  function drawImg() {
    ...
    if(rouletteType===CASINO){
        ...
        drawText(deg+sliceDeg/2, casinolabel[i]);
    }
    else if(rouletteType=TRIAL){
        drawSlice(deg, trialcolor[i]);
        drawText(deg+sliceDeg/2, triallabel[i]);
    }
      deg += sliceDeg;
    }
  }
...
function indicateRoulleteResult(){
  if(rouletteType===CASINO){
    android_toast(casinolabel[game.roullete_result])
  }
  else if(rouletteType===TRIAL){
    android_toast(triallabel[game.roullete_result])
  }
}
