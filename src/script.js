...
const EFFECT_DESC = [
  ...
  "[Debt] Can`t earn money and sell tokens, -18$ instead",
...
function playerDie(
  ...
  if (turn === game.myturn) {
    text = "You Died!"
    if (LANG === "kor") text = "적에게 당했습니다"
    if (skillfrom == -1) {
      text = "Executed!"
      if (LANG === "kor") text = "처형되었습니다"
    }
    if (isShutDown) {
      text = "Shut Down!"
      if (LANG === "kor") text = "제압되었습니다"
      largetext = false
    }
    switch (killerMultiKillCount) {
      ...
      case 2:
        ...
        if (LANG === "kor") text = "적 더블킬"
        largetext = false
        break
      case 3:
        text = "ENEMY TRIPLE KILL!"
        if (LANG === "kor") text = "적 트리플킬"
        largetext = true
        break
      case 4:
        text = "ENEMY QUADRA KILL!"
        if (LANG === "kor") text = "적 쿼드라킬"
        largetext = true
        break
      case 5:
        text = "ENEMY PENTA KILL!"
        if (LANG === "kor") text = "적 펜타킬"
        largetext = true
        break
      default:
        text = "ENEMY IS LEGENDARY!"
        if (LANG === "kor") text = "적은 전설적입니다"
        largetext = true
        break
    }
    if (largetext) {
    ...
    if (LANG === "kor") text = "적을 처치했습니다<br>주사위 한번더!"
    if (isShutDown) {
      text = "Shut Down!"
      if (LANG === "kor") text = "제압되었습니다"
      largetext = true
    ...
    switch (killerMultiKillCount) {
      ...
      case 2:
        text = "DOUBLE KILL!"
        if (LANG === "kor") text = "더블킬"
        largetext = false
        break
      case 3:
        text = "TRIPLE KILL!"
        if (LANG === "kor") text = "트리플킬"
        largetext = true
        break
      case 4:
        text = "QUADRA KILL!"
        if (LANG === "kor") text = "쿼드라킬"
        largetext = true
        break
      case 5:
        text = "PENTA KILL!"
        if (LANG === "kor") text = "펜타킬"
        largetext = true
        break
      default:
        text = "LEGENDARY!"
        if (LANG === "kor") text = "전설의 출현!"
        largetext = true
        break
    }
    if (largetext) {
    ...
    if (
      game.teams[skillfrom] !== "none" &&
      game.teams[turn] === game.teams[game.myturn] &&
      killerMultiKillCount > 2
    ) {
      text = "Ally"
      if (LANG === "kor") text = "아군"
    }
    ...
    switch (killerMultiKillCount) {
      ...
      case 1:
        if (isShutDown) {
          if (LANG === "kor") text += "이 제압되었습니다"
          else text += " Shut Down!"
          largetext = false
        } else {
          if (LANG === "kor") text += "이 사망했습니다"
          else text += "Enemy died!"
        }
        break
      case 2:
        if (LANG === "kor") text += " 더블킬"
        else text += " DOUBLE KILL!"

        largetext = false
        break
      case 3:
        if (LANG === "kor") text += " 트리플킬"
        else text += " TRIPLE KILL!"

        largetext = true
        break
      case 4:
        if (LANG === "kor") text += " 쿼드라킬"
        else text += " QUADRA KILL!"
        largetext = true
        break
      case 5:
        if (LANG === "kor") text += " 펜타킬"
        else text += " PENTA KILL!"
        largetext = true
        break
      default:
        if (LANG === "kor") text += "은 전설적입니다"
        else text += " IS LEGENDARY!"
        largetext = true
        break
    }
    if (largetext) {
...
function playerRespawn(turn, respawnPos) {
